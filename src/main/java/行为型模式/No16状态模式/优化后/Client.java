package 行为型模式.No16状态模式.优化后;

import 行为型模式.No16状态模式.优化后.状态角色.StoppingState;
import 行为型模式.No16状态模式.优化后.环境角色.Context;

/**
 * 定义：
 *   对有状态的对象，把复杂的“判断逻辑”提取到不同的状态对象中，允许状态对象在其内部状态发生改变时改变其行为。（在实现上有点类似于双向链表0
 *
 * 状态模式包含以下主要角色。
...
public class Client {
    public static void main(String[] args) {
        ...
        //以下方法执行后，环境类中的状态也会随之改变，不需要再手动重新设置
        context.stop();
        context.open();
        context.close();
        context.run();
